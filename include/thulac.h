...
std::vector<std::string> eqSeg(const std::string &s, int n) {
    std::vector<std::string> output;
    int str_len = s.size();
    int per_len = str_len / n;
    for(int i=0; i<n; i++) {
        output.push_back(s.substr(i * per_len, per_len));
    }
    return output;
}
...
THULAC_result THULAC::cut(const std::string& in) const {
